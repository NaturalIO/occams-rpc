...
pub fn client_task_impl(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    ) = (&res_field, &noti_field)
    {
        ...
        quote! {
            impl #impl_generics razor_stream::client::task::ClientTaskGetResult<#error_type> for #struct_name #ty_generics #where_clause {
                #[inline]
                fn get_result(&self) -> Result<(), &razor_rpc_core::error::RpcError<#error_type>> {
                    match self.#res_field_name.as_ref() {
                        Some(Ok(())) => Ok(()),
                        Some(Err(e)) => Err(e),
                        None => Err(&razor_rpc_core::error::RpcError::Rpc(razor_rpc_core::error::RpcIntErr::Internal)),
                    }
                }
            }
            ...
            impl #impl_generics razor_stream::client::task::ClientTaskDone for #struct_name #ty_generics #where_clause_with_into {
                #[inline]
                fn set_custom_error<C: razor_rpc_core::Codec>(&mut self, codec: &C, res: razor_rpc_core::error::EncodedErr) {
                    let rpc_error = match res {
                        razor_rpc_core::error::EncodedErr::Rpc(e) => e.into(),
                        razor_rpc_core::error::EncodedErr::Num(n) => {
                            if let Ok(e) = <#error_type as razor_rpc_core::error::RpcErrCodec>::decode(codec, Ok(n as u32)) {
                                e.into()
                            } else {
                                razor_rpc_core::error::RpcIntErr::Decode.into()
                            }
                        }
                        razor_rpc_core::error::EncodedErr::Static(s) => {
                            if let Ok(e) = <#error_type as razor_rpc_core::error::RpcErrCodec>::decode(codec, Err(s.as_bytes())) {
                                e.into()
                            } else {
                                razor_rpc_core::error::RpcIntErr::Decode.into()
                            }
                        }
                        razor_rpc_core::error::EncodedErr::Buf(b) => {
                            if let Ok(e) = <#error_type as razor_rpc_core::error::RpcErrCodec>::decode(codec, Err(&b)) {
                                e.into()
                            } else {
                                razor_rpc_core::error::RpcIntErr::Decode.into()
                            }
                ...
                #[inline]
                fn set_rpc_error(&mut self, e: razor_rpc_core::error::RpcIntErr) {
    ...
    let expanded = quote! {
        ...
        impl #impl_generics_for_impl razor_stream::client::task::ClientTaskEncode for #struct_name #ty_generics_for_impl #where_clause_for_impl {
            #[inline]
            fn encode_req<C: razor_rpc_core::Codec>(&self, codec: &C, buf: &mut Vec<u8>) -> Result<usize, ()> {
        ...
        impl #impl_generics_for_impl razor_stream::client::task::ClientTaskDecode for #struct_name #ty_generics_for_impl #where_clause_for_impl {
            #[inline]
            fn decode_resp<C: razor_rpc_core::Codec>(&mut self, codec: &C, buffer: &[u8]) -> Result<(), ()> {
